...
if [ "${label}" = FALSE ]; then 
	...
	templatefile=${templatefile:-"sub-0001_acq-3T2mm_contrast_oddball_zscored.dscalar.nii"}
	templatefolder=${templatefolder:-"/home/yaco0006/shared/projects/Baby7T/analysis/oddball_task/"}
	old_templatefolder=${old_templatefolder:-"/home/yaco0006/shared/projects/Baby7T/analysis/oddball_task/"}
	old_templatefile=${old_templatefile:-"sub-0001_acq-3T2mm_contrast_oddball_zscored.dscalar.nii"}
...
#        echo "      ${wb_command} -scene-capture-image ${temp_scene} ${scenenum} ${out} 900 800 > /dev/null 2>&1"
#        ${wb_command} -scene-capture-image ${temp_scene} ${scenenum} ${out} 900 800 > /dev/null 2>&1
	...
	if [ "${make_quad}" = TRUE ]; then #make DV
		echo "Generating Dorsal-Ventral pics"
        	...
        	echo "Calling wb_command as follows:"
        	echo "      ${wb_command} -scene-capture-image ${outputfilename} ${shortscenename} ${image_file_name} 944 710 "
        	${wb_command} -scene-capture-image ${outputfilename} ${shortscenename} ${image_file_name} 944 710
fi
...
if [ "${make_subcorticals}" = TRUE ]; then 
    ...
    echo "Label is set to FALSE in subcortical section."
		...
		temp_scene_sub=${temp_scene_sub:-"/home/exacloud/lustre1/fnl_lab/code/internal/utilities/make_dscalar_pics/MSC01_template_scene_subcort_scalar_MSI.scene"}
		
    templatefile_sub=${templatefile:-"sub-0001_acq-3T2mm_contrast_oddball_zscored.dscalar.nii"}
		templatefolder_sub=${templatefolder:-"/home/yaco0006/shared/projects/Baby7T/analysis/oddball_task/"}
		old_templatefolder_sub=${old_templatefolder_sub:-"/projects/standard/faird/shared/projects/HCP-Dv2_net_template_matching/probability_maps/ages08-09/"}
		old_templatefile_sub=${old_templatefile_sub:-"10minute_ages08-09_Aud_network_probability.dscalar.nii"}
    ...
    threshold_inout_default=${threshold_inout_default:-"THRESHOLD_TEST_SHOW_OUTSIDE"}
		...
		old_templatefolder_sub=${old_templatefolder_sub:-"/home/yaco0006/shared/projects/Baby7T/analysis/oddball_task/"}
		old_templatefile_sub=${old_templatefile_sub:-"sub-0001_acq-3T2mm_contrast_oddball_zscored.dscalar.nii"}
