...
continuous=${1}; shift # set true if continous data, pallet is assumed to be Power Colors.  If true, scene file will use RGB colors.
continuous_lower=${1}; shift # lower scale limit for color pallet
continuous_upper=${1}; shift # upper scale limit for color pallet

threshold_image=${1}; shift # set true if you want to exclude data outside/inside thresholds, pallet is assumed to be Power Colors.  If true, scene file will use RGB colors.
threshold_lower=${1}; shift # lower threshold for color pallet
threshold_upper=${1}; shift # upper threshold for color pallet
...
continuous_upper_default=${1}; shift #the default value for the color pallet upper scale limit, will be replaced with the new value
continuous_lower_default=${1}; shift #the default value for the color pallet lower scale limit, will be replaced with the new value
...
threshold_lower_default=${1}; shift #the default value for the color pallet lower threshold, will be replaced with the new value
threshold_upper_default=${1}; shift #the default value for the color pallet upper threshold, will be replaced with the new value
...
image_file_name=${outputfolder}/${shortfilename}.png
...
if [ "${continuous}" = TRUE ]; then 
	...
	#Reference Scene file:
	temp_scene=${temp_scene:-"/home/exacloud/lustre1/fnl_lab/code/internal/utilities/make_dscalar_pics/template_very_inflated.scene"}
	templatefile=${templatefile:-"MSC01_half2_SMOOTHED_2.55.dtseries.nii_all_frames_at_FD_0.2_20i_prefsurface_only_pajek_2perc_20dist_bin_No_COMMUNITIES_recolored.dscalar.nii"}
	templatefolder=${templatefolder:-"/home/exacloud/lustre1/fnl_lab/projects/MSC_to_DCAN/analyses/infomap/Community_Detection_Min_Dist_20_TieDen_0.02_MinNet_Size_400_MinReg_Size_30/MSC01_half2_SMOOTHED_2.55.dtseries.nii_all_frames_at_FD_0.2_20i_prefsurface_only.dconn.nii/wb_ready_files/"}
	templatescenename=${templatescenename:-"MSC01_template_scene"}
fi
...
#        ${wb_command} -show-scene ${temp_scene} ${scenenum} ${out} 900 800 > /dev/null 2>&1
	...
	if [ "${make_quad}" = TRUE ]; then #make DV
		echo "Generating Dorsal-Ventral pics"
        	echo "Calling wb_command as follows:"
        	echo "      ${wb_command} -show-scene ${outputfilename} ${shortscenename} ${image_file_name} 1310 718 "
        	${wb_command} -show-scene ${outputfilename} ${shortscenename} ${image_file_name} 1310 718
...
if [ "${make_subcorticals}" = TRUE ]; then 
	...
	image_file_name_PA=${outputfolder}/${shortfilename}_PA.png
	image_file_name_AX=${outputfolder}/${shortfilename}_AX.png
	image_file_name_CO=${outputfolder}/${shortfilename}_CO.png
	...
	if [ "${continuous}" = TRUE ]; then 
		...
		temp_scene_sub=${temp_scene_sub:-"/home/exacloud/lustre1/fnl_lab/code/internal/utilities/make_dscalar_pics/template_very_inflated.scene"}
		templatefile=${templatefile:-"MSC01_half2_SMOOTHED_2.55.dtseries.nii_all_frames_at_FD_0.2_20i_prefsurface_only_pajek_2perc_20dist_bin_No_COMMUNITIES_recolored.dscalar.nii"}
		templatefolder=${templatefolder:-"/home/exacloud/lustre1/fnl_lab/projects/MSC_to_DCAN/analyses/infomap/Community_Detection_Min_Dist_20_TieDen_0.02_MinNet_Size_400_MinReg_Size_30/MSC01_half2_SMOOTHED_2.55.dtseries.nii_all_frames_at_FD_0.2_20i_prefsurface_only.dconn.nii/wb_ready_files/"}
		templatescenename=${templatescenename:-"MSC01_template_scene"}
	fi
	...
	kathy=${templatefolder}
	kathy2=${subjectfoldername}
	#sed -i "s!${templatefolder}_PATH!${subjectfoldername}!g" $outputfilename_sub
	...
	if [ "${continuous}" = TRUE ]; then #scale
		...
		#if (( $continuous_lower_default < 0 )); then 
		if [ "$continuous_lower" -ge 0 ] && [ "$continuous_upper" -gt 0 ] ; then  #-lt = "less than". If the lower or upper limit both positive, turn on zero and turn off negative in scale.
			echo "Replace scene positive display setting: " ${continuous_pos_default} "with true</DisplayPositiveData>"
			kathy_posa=${continuous_pos_default}
			kathy_posb="true</DisplayPositiveData>"
		...
		if [ "$continuous_lower" -lt 0 ] && [ "$continuous_upper" -ge 0 ] ; then  #-lt = "less than". If the lower is negative and the upper is positive, turn on everything in scale.
			echo "Replace scene positive display setting: " ${continuous_pos_default} "with true</DisplayPositiveData>"
			kathy_posa=${continuous_pos_default}
			kathy_posb="true</DisplayPositiveData>"
			sed -i 's,'"${kathy_posa}"','"${kathy_posb}"',g' "$outputfilename_sub"
		...
		if [ "$continuous_lower" -lt 0 ] && [ "$continuous_upper" -lt 0 ] ; then  #-lt = "less than". If the lower and upper limit are neagtive, turn off positive+zero in scale.
			echo "Replace scene positive display setting: " ${continuous_pos_default} "with false</DisplayPositiveData>"
			kathy_posa=${continuous_pos_default}
			kathy_posb="false</DisplayPositiveData>"
			sed -i 's,'"${kathy_posa}"','"${kathy_posb}"',g' "$outputfilename_sub"
	...
	# Make the pictures
			...
			echo "Generating Subcortical pics"
	        	echo "Calling wb_command as follows:"
	        	echo "      ${wb_command} -show-scene ${outputfilename_sub} ${shortscenename} ${image_file_name_PA} 1471 718 "
	        	${wb_command} -show-scene ${outputfilename_sub} ${shortscenename_PA} ${image_file_name_PA} 1471 718
	
	        	echo "Calling wb_command as follows:"
	        	echo "      ${wb_command} -show-scene ${outputfilename_sub} ${shortscenename} ${image_file_name_AX} 1471 718 "
	        	${wb_command} -show-scene ${outputfilename_sub} ${shortscenename_AX} ${image_file_name_AX} 1417 718
	
	        	echo "Calling wb_command as follows:"
	        	echo "      ${wb_command} -show-scene ${outputfilename_sub} ${shortscenename} ${image_file_name_CO} 1471 718 "
	        	${wb_command} -show-scene ${outputfilename_sub} ${shortscenename_CO} ${image_file_name_CO} 1417 718
