...
wait_time=5; %set to 15 or more if you're running into issues where matlab tries to read the data before it's done writing the data.
...
elseif strcmpi(series, 'dtseries')
    ...
    if exist('memory_limit_value', 'var') ==1
        if isnumeric(memory_limit_value) ==1
            disp(['memory limit value is; ' num2str(memory_limit_value) ])
            
        else
            disp(['memory limit value is; ' memory_limit_value ])
        end
    else
    end
    
    if exist('memory_limit_value', 'var') ==1 && strcmp(memory_limit_value,'none') ~=1
        ...
        if other_motion_mask 
            try
                ...
                FDvec = remove_outliers_if(remove_outliers, FDvec, ...
                    outlier_rmv_file, stdev_temp_filename, wb_command, wait_time);
            catch
...
function [A_smoothed_i, outfile] = minutes_limit_calculation(...
        ...
        memory_limit_option, memory_limit_value)
    % Whether to Fisher-z transform the correlation values (wb_command's
    % -fisher-z flag) before writing the dconn. Controlled by the FISHER_Z
    % environment variable (set via run_PCM.sh's --fisher-z flag, exported to
    % every matlab invocation) -- default on (any value other than exactly
    % '0'), matching this function's long-standing hardcoded-on behavior, so
    % nothing changes for callers that don't set FISHER_Z at all.
    if strcmp(getenv('FISHER_Z'), '0')
        fisher_z_flag = '';
    else
        fisher_z_flag = ' -fisher-z';
    end
    ...
    if strcmpi(smoothing, 'none')
        ...
        if ~exist(A_smoothed_i, 'file')
            ...
            'smoothing. This may take a few minutes.']);
    ...
    if ~exist(outfile, 'file') % check to see if the file already exists
        if strcmpi(smoothing, 'none')
            ...
            cmd = [wb_command ' ' precision ...
                   ...
                   weights fisher_z_flag memory_limit_option ' ' num2str(memory_limit_value)];
        else
            if ~exist('weights', 'var')
                weights = [' -weights ' char(output_dir) ...
                           char(orig_motion_file) '_' ...
            ...
            cmd = [wb_command ' ' precision ' -cifti-correlation ' ...
                   ...
                   suffix2 weights fisher_z_flag memory_limit_option ' ' num2str(memory_limit_value)];
        end
        execute_display_and_clear(cmd, ['Running wb_command cifti-' ...
...
function FDvec = remove_frames_from_FDvec(cifti_file, FDvec, ...
    ...
    cmd = [wb_command ' -cifti-stats ' char(cifti_file) ...
           ' -reduce STDEV > ' stdev];
       disp(['calling wb_command as follows:' cmd])
    system(cmd);
    if wait > 0
    ...
    try
        ...
        disp(['error loading stdev file:' stdev '. Trying again by reading directly from wb_command output...'])
        [status,cmdout] = system([wb_command ' -cifti-stats '  char(cifti_file) ' -reduce STDEV']);
        ...
        STDEV_file = cell2mat(textscan(cmdout, '%f\n'));
    end
