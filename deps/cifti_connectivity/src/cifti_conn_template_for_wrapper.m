function cifti_conn_template_for_wrapper(wb_command, dt_or_ptseries_conc_file, series, motion_file, FD_threshold, TR, minutes_limit, smoothing_kernel, left_surface_file, right_surface_file, bit8, remove_outliers, additional_mask, make_dconn_conc, output_directory, dtseries_conc, use_continous_minutes, memory_limit_value, keep_conn_matrices, conn_output_filename, add_but_donot_divide)

%This code generates an average correlation matrix ("or template"), from a
%conc files of dtseries (and motion files, etc).  Idt does this by generating a
%correlation matric for each subject. Each subject is added to the running
%matrix.  After all the indivudal subject's matrices are added, the code 
%then divides the final matrix by the total bumber of subjects.
...
% zero (of note as of 11/20/2017 having this on for this code has not been tested - df)
...
% output_directory = the output directory of the files that you make
% dtseries_conc =  the path to a dtseries confile.  This is important if you're using outlier deterction and you use a ptseries.  YOu'll want to calculate outliers baed on the dtseries.
% keep_conn_matrices = if set to 1 this will remove the dconns that were created.%  NOTE: this will delete dconns that have been previously made if the name matches the expected output.
% conn_output_filename = if you want to change the final output name, then provide the full file name including the path here.
% add_but_donot_divide = set this option to 1 if you want to add all the
% matrices together but do not want to divide by the number of matrices.
% use_continous_minutes = set to 1 to use a continous amount of data from the mask, rather than randomly sampling throughout the time series.
% memory_limit_value = set memory limit for building a your correlation matrix (in GB). If you have access to lots of memory, set this to 'none'.

%Outputs are:
%A correlation matrix that is the average of all the timseries data that
%you've provided. 
...
%% 04/09/2024 -RH
%Added additional documentation for options that have been added to the cifti conn matrix code.
% updated the call to the call the cifti_conn_matrix_continous function.
%added an option to "add but do not divide".
...
add_but_donot_divide = make_numeric(add_but_donot_divide,'add_but_donot_divide');
...
remove_outliers = make_numeric(remove_outliers, '(remove_outliers');
...
if remove_outliers ==1
    if strcmpi('conc', conc)
        E = importdata(dtseries_conc);
    else
        E = {dtseries_conc};
    end
else
    if strcmp(dtseries_conc,'none') ==1
        disp('Setting outlier conc to timeseries conc, even though it will not be used. to preventthe code from breaking later')
        if strcmpi('conc', conc)
            E = importdata(dt_or_ptseries_conc_file);
        else
            E = {dt_or_ptseries_conc_file};
        end
    else
        if strcmpi('conc', conc)
            E = importdata(dtseries_conc);
...
if exist(conn_output_filename,'file') ==0
...
%         output_directory, dtseries_conc));
    [dconn_outfile_path, motion_mask_path] = cifti_conn_matrix_for_wrapper_continous(wb_command, ...
    A{1}, series, motion_B, num2str(FD_threshold), TR, ...
    minutes_limit, smoothing_kernel, left, right,...
    bit8, remove_outliers, additional_mask,  ...
    make_dconn_conc, output_directory, E{1},use_continous_minutes,memory_limit_value);
else
...
cmd = ['mv ' dconn_outfile_path ' ' conn_output_filename ];
system(cmd);
else
    disp('Dconn file already found.');
end

% Rename average dconn, do cifti math, clean variables, then do next
% if isempty(paths)
%     rename_avg_dconn(dconn_outfile_path, output_conc, keep_conn_matrices, suffix2);
% else
...
for i = 2:length(A)
    ...
    if isempty(paths)
        ...
        [dconn_outfile_path, motion_mask_path] = cifti_conn_matrix_for_wrapper_continous( ...
            wb_command, A{i}, series, motion_B, num2str(FD_threshold), ...
            TR, minutes_limit, smoothing_kernel, left, right, bit8, ...
            remove_outliers, additional_mask, make_dconn_conc, ...
            output_directory, E{i},use_continous_minutes,memory_limit_value);
    else
    ...
    % Rename average dconn, do cifti math, clean variables, then do next
    %rename_avg_dconn(dconn_paths, output_conc, keep_conn_matrices,suffix2);
    cifti_math_and_cleanup(dconn_outfile_path, conn_output_filename, ...
...
if add_but_donot_divide ==0
%mean = s1/N
cmd = [wb_command ' -cifti-math " s1/' N '" ' conn_output_filename ' -var s1 ' conn_output_filename ];
execute_and_clear(cmd);
end


disp(['Done making average template: ' conn_output_filename ]);
end
...
function num_var = make_numeric(num_var, var_name)
    ...
    elseif is_none(num_var) ==0 && ~isnumeric(num_var)
        num_var = str2num(num_var);
    end
end
...
function surface_files = get_surface_files(LR, surface_conc, conc)
    ...
    for i = 1:length(surface_files)
        ...
        disp(' There was a problem checkin ght path of your surface files.  You may be using the same one for each subject')
    end
...
function cifti_math_and_cleanup(dconn_paths, output_conn, ...
                                keep_conn_matrices, suffix2, wb_command)
    % Run cifti math on average matrices, and then delete dconn files 
    % unless keep_conn_matrices is true
    cmd = [wb_command ' -cifti-math  " M1 + M2 " ' output_conn ' -var M1 ' output_conn '  -var M2 ' dconn_paths];
    execute_and_clear(cmd);
    if keep_conn_matrices == 0
        rm_cmd= ['rm -f ' dconn_paths];
        disp(rm_cmd);
        system(rm_cmd);
    else
    end
