...
if [ "${PIPELINE}" == "xcpd" ]; then
    ...
    motion_file_mat="${DERIVATIVESDIR}/sub-${SUB}/ses-${SES}/func/sub-${SUB}_ses-${SES}_task-${TASK}_desc-abcc_qc_power_2014_FD_only.mat"
    motion_file_hdf5="${DERIVATIVESDIR}/sub-${SUB}/ses-${SES}/func/sub-${SUB}_ses-${SES}_task-${TASK}_desc-abcc_qc.hdf5"
...
if [ ${SURF_ONLY} == 0 ]; then
  ...
  echo "template is ${TEMPLATE_PATH}"
    ...
    /projects/standard/faird/shared/code/internal/analytics/TemplateShuffle/code/functions/twins_mapping_wrapper_washu_nordic.sh \
        ...
        ${FD} \
        "${dtseries_in}" \
        "${work_dir}/sub-${SUB}_ses-${SES}_task-${TASK}_desc-abcc_qc_power_2014_FD_only.mat" \
        ${surf_L} \
        ${surf_R} \
