...
def plot_data(exp_data, fig_dir, plot_type='line', coefficient_type='DiceCoefficient', threshold='0.8', summary_only=False, data_prefix='ExpData-'):
    ...
    # Plot data for each network individually
    if not summary_only:
        for net in networks:
    ...
    # Plot combined "all networks" data
    plt.figure()
    for net in networks:
        data = network_data[net]
        x_positions = list(range(len(data)))
    ...
    # Handle the case where the data_prefix is not properly removed
    #exp_times = sorted(exp_data.keys(), key=lambda x: int(x.replace(data_prefix, '').split('-')[0].replace('m', '')))
    exp_times = sorted(exp_data.keys(), key=lambda x: int(x.replace(data_prefix, '').split('m')[0].split('-')[-1]))
