function extract_minutes_dtseries_and_motion_and_tsv(infile, motionfile, outfolder, TR, minutes, keep_from, tsv_files, runperm)
    ...
    % Process the .tsv files if provided
    if nargin >= 7 && ~isempty(tsv_files)
        for i = 1:length(tsv_files)
            tsv_file = tsv_files{i};
            [~, tsv_name, tsv_ext] = fileparts(tsv_file);

            % Read the tsv file with headers
            tsv_table = readtable(tsv_file, 'FileType', 'text', 'Delimiter', '\t', 'ReadVariableNames', true);

            % Number of data rows
            num_data_rows = height(tsv_table);

            % Check that num_data_rows matches total_TRs
            if num_data_rows ~= total_TRs
                error('The number of data rows in %s (%d) does not match the number of TRs in the dtseries (%d).', tsv_file, num_data_rows, total_TRs);
            end

            % Extract the specified TR range
            new_tsv_table = tsv_table(start_TR:end_TR, :);

            % Construct new tsv filename
            new_tsv_name = sprintf('%s-%sminutes%s', tsv_name, num2str(minutes), tsv_ext);
            new_tsv_file = fullfile(outfolder, new_tsv_name);

            % Save the new tsv file
            writetable(new_tsv_table, new_tsv_file, 'FileType', 'text', 'Delimiter', '\t');
            fprintf('Saved new tsv file: %s\n', new_tsv_file);
        end
    end
end

function [SUB, SES, TASK] = extract_sub_ses_task_from_path(filepath)
