function motion_and_dtseries_dconn_testing(SUB, SES, FD, TASK, TR, MAXMIN, permnum, infolder, infile, BASEDIR, intrp_noise, shuffle_option, shuffle_chunk_size, holdout_pct)
...
for i = 1:size(MIN_X, 2)
    % Calculate the frame count for the current minute
    framecount = round(MIN_X(i) * 60 / TR);
    
    % Check if there are enough good frames for the current framecount
    if framecount > numel(good_frames)
        fprintf('Not enough clean data to make masks up to %d minute(s) which is lower than %d (MAXMIN). ', MIN_X(i), MAXMIN);
        fprintf('Only making masks up to %d minute(s).\n', MIN_X(i - 1));
        break; % Exit the loop
    end
    ...
    % Update the mask for good frames up to framecount
