...
%    parameters etc.
    ...
    patch_colors = [ 1.000 0.161 0.161;
                     0.165 0.157 0.678;
                     0.914 0.910 0.161;
                     0.165 0.808 0.153;
                     0.204 0.592 0.678;
    ...
    for k_left = 1:size(data, 1) % for each row
        color_index = min(k_left, num_colors);
        color = patch_colors(color_index, :);
        ...
        patch('XData', X, 'YData', Y, 'FaceColor', color, 'FaceAlpha', .6, 'EdgeColor', 'none');
    end % for each row
    
    % Place left labels
    text(zeros(1, size(data, 1)) - 0.02, ...
         left_category_points + left_category_sizes./2, ...
         left_labels, 'FontSize', 14, 'HorizontalAlignment', 'right', 'VerticalAlignment', 'middle', 'Rotation', 0);
     
    % Place right labels
    text(ones(1, size(data, 2)) + 0.02, ...
         right_category_points + right_category_sizes./2, ...
         right_labels, 'FontSize', 14, 'HorizontalAlignment', 'left', 'VerticalAlignment', 'middle', 'Rotation', 0);
