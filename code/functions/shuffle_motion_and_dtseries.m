function shuffle_motion_and_dtseries(SUB,SES,FD,TASK,TR,MAXMIN,permnum,infolder,infile,BASEDIR,intrp_noise,shuffle_option,shuffle_chunk_size,holdout_pct)
...
%shuffle_chunk_size = 10; % Percentage for chunk size if 'percent' option is chosen
...
% intrp_noise=0
% %shuffle_option = 'percent'; % Options: 'run' or 'percent'
% shuffle_chunk_size = 15; % Percentage for chunk size if 'percent' option is chosen
...
elseif strcmp(shuffle_option, 'minutes')
    % Calculate chunk size in TRs based on the input minutes
    chunk_size_TRs = round((shuffle_chunk_size * 60) / TR);  % Convert minutes to seconds and divide by TR to get TRs
    num_chunks = ceil(total_TRs / chunk_size_TRs);
    
    % Check if the shuffled order mat file exists for this chunk size
    ...
    if exist(shuffled_mat_path, 'file')
        ...
        % Create shuffled order for minute chunks
    ...
    % Use the appropriate permutation from the shuffled minute order
    minute_order = shuffled_minute_order(permnum, :);

    % Divide data into chunks and shuffle
    chunk_sizes = repmat(chunk_size_TRs, 1, num_chunks);
    chunk_sizes(end) = total_TRs - chunk_size_TRs * (num_chunks - 1); % Adjust last chunk size if necessary
    data_chunks = mat2cell(data, size(data, 1), chunk_sizes);
...
elseif strcmp(shuffle_option, 'TRs')
    % Each TR is treated as a chunk
    num_TRs = total_TRs;  % Total number of TRs in the dataset
    num_chunks = num_TRs;  % Each TR is a chunk
    
    % Define the path to the .mat file for storing shuffled TR order
    shuffled_mat_path = fullfile(BASEDIR, ['shuffled_TR_order_sub-' SUB '_ses-' SES '.mat']);
    
    if exist(shuffled_mat_path, 'file')
        % Load the existing shuffled TR order
        load(shuffled_mat_path, 'shuffled_TR_order');
    else
        % Create shuffled TR order
        shuffled_TR_order = zeros(num_permutations, num_chunks);
        for i = 1:num_permutations
            shuffled_TR_order(i, :) = randperm(num_chunks);
        end
        % Save the shuffled TR order
        save(shuffled_mat_path, 'shuffled_TR_order');
    end

    % Use the appropriate permutation from the shuffled TR order
    TR_order = shuffled_TR_order(permnum, :);

    % Shuffle TRs
    rearranged_data = data(:, TR_order);  % Rearrange columns (TRs) based on shuffled order
...
elseif strcmp(shuffle_option, 'minutes')
    ...
    % Following similar steps for motion data as in 'percent' handling
    motion_chunk_sizes = chunk_sizes;  % Align motion data chunk sizes with DT series data chunks
    fd_chunks = mat2cell(retained_frames, motion_chunk_sizes, 1);
    shuffled_fd_chunks = fd_chunks(minute_order);
    rearranged_fd = cat(1, shuffled_fd_chunks{:});
    
elseif strcmp(shuffle_option, 'TRs')
    % Shuffle corresponding motion data (fd_vector)
    rearranged_fd = retained_frames(TR_order);  % Rearrange frame removal vector similarly
