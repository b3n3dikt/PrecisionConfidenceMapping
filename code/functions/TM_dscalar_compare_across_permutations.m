...
if make_NetConfMaps == 1
    ...
    fullFilePaths = fullfile({fileListStruct.folder}, fileNames);
    lowercolor=1;
    % Fixed at 18 to match the 'power_surf' colorscheme below (a fixed 1-18 Power/ABCC
    % network palette). NOT num_networks (line 26) -- that's how many of the 18 network IDs
    % are actually present in one subject's data (often fewer than 18), not the palette's
    % scale ceiling. Using a data-dependent value here compresses the color scale so the same
    % network ID renders a different color than the fixed palette intends.
    uppercolor=18;
    lowerthresh=0.1;
    upperthresh=30;
