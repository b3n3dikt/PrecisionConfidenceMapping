function calculate_dice_coefficient_PCM(BASEDIR, dscalarswithassignments1, dscalarswithassignments2, network_name, ConfMap, threshold, fig_dir)
...
%% Extracting cortical and subcortical 
...
DC = zeros(1, num_permutations);
cDC = zeros(1, num_permutations);
...
for s = 1:length(dscalar_list1)
    ...
    if ConfMap
        ...
        cDC(s) = continuous_Dice_coefficient(Mdata, Cdata_bin);

        % Calculate Dice coefficient (DC)
        DC(s) = Dice_coefficient(Mdata_bin, Cdata_bin);
        %% Calc network transition matrix
...
writematrix(avg_DC, fullfile(network_folder, 'Average_DiceCoefficient.txt'));
writematrix(PPV, fullfile(network_folder, 'PositivePredictiveValue.txt'));
writematrix(NPV, fullfile(network_folder, 'NegativePredictiveValue.txt'));
writematrix(network_stability,fullfile(network_folder, 'NetworkStability.txt'));
writematrix(FalsePositive, fullfile(network_folder, 'FalsePositive.txt'));
writematrix(FalseNegative, fullfile(network_folder, 'FalseNegative.txt'));
writematrix(TruePositive, fullfile(network_folder, 'TruePositive.txt'));
writematrix(TrueNegative, fullfile(network_folder, 'TrueNegative.txt'));


writematrix(std_DC, fullfile(network_folder, 'STDEV_DiceCoefficient.txt'));
writematrix(DC, fullfile(network_folder, 'DiceCoefficientsMatrix.txt'));
writematrix(avg_cDC, fullfile(network_folder, 'Average_continuousDiceCoefficient.txt'));
writematrix(std_cDC, fullfile(network_folder, 'STDEV_continuousDiceCoefficient.txt'));
writematrix(cDC, fullfile(network_folder, 'continuousDiceCoefficientsMatrix.txt'));
