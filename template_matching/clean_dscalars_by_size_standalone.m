function [outname] = clean_dscalars_by_size_standalone(dscalarwithassignments,manualset,groupnetworksfile,dostripes,mincol,minsize,orig_parcelsfile,make_consensus,assign_unassigned,remove46)
%consensus_maker_knowncolors(regularized_ciftifile,[manualset],[groupnetworksfile],[dostripes],[mincol],[minsize],[orig_parcelsfile])
%
% PCM's fork of clean_dscalars_by_size (renamed from the lab-internal name
% clean_dscalars_by_size_copy_jm_msi — see CHANGES.md). Nearly identical to
% the fetched deps/matlab_tm/clean_dscalars_by_size.m (same ~500 lines of
% network-cleaning logic; the only functional gap is the upstream version's
% extra removescan17 argument, unused by PCM). Kept as PCM's own file rather
% than delegating to the fetched version because the fetched function does
% its OWN internal path/settings setup via settings_comparematrices() (the
% untouched upstream settings file), which hard-errors
...
% STANDALONE PCM: self-locate via mfilename, not which('clean_dscalars_by_size').
% The original used which() on the SIBLING function's name (a pre-existing copy
% artifact from when this file was forked off clean_dscalars_by_size.m) which
% only happened to work by coincidence when both files lived in the same
% folder. Now that the vanilla clean_dscalars_by_size.m comes from the fetched
% deps/matlab_tm/ dependency (see config.sh MATLAB_TM_DIR) while this file
% stays in PCM's own template_matching/, which('clean_dscalars_by_size') would
% resolve to the WRONG directory and support_folder below would point at
% deps/matlab_tm/support_files instead of this file's own support_files/.
this_code = mfilename('fullpath');
...
if isdeployed
    ...
    % STANDALONE PCM: removed hardcoded rmpath/addpath to MSCcodebase — not
    % bundled, and not needed (was cleaning up a non-working gifti path from a
    % codebase PCM doesn't ship).
