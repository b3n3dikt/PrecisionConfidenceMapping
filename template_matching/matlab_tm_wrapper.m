function matlab_tm_wrapper(TR,FD_threshold,dt_or_ptseries_conc_file,motion_file,left_surface_file, right_surface_file, output_file_name, cifti_output_folder,template_path,surface_only,already_surface_only,output_directory, dtseries_conc,additional_mask, run_infomap_too)
% PCM's per-permutation driver for METHOD=matlab_tm: takes a (possibly
% bootstrapped/shuffled) subject's dtseries + motion + surfaces, runs it
% through template_matching_RH (fetched from DCAN-Labs/
% compare_matrices_to_assign_networks by code/setup_matlab_tm.sh), and
% writes the network-assignment cifti. Originally derived from
% R. Hermosillo's twins_mapping_wrapper (1/8/2019), substantially rewritten
% since — see CHANGES.md for PCM-specific history.
%
...
% Default on, matching this wrapper's original hardcoded behavior; set
...
this_code = which('matlab_tm_wrapper');
...
% STANDALONE PCM: Removed server-specific rmpath/addpath calls.
% plotting-tools and MergeTimeSeries are only used in the infomap branch
% (use_all_ABCD_tasks==1), which PCM does not invoke.
warning('on')
...
for i = 1:length(dtseries_file) %number of subjects
    ...
    % STANDALONE PCM: template_matching dir already on path via which()-based
    % self-location at top of this function (code_dir) and via MATLAB_ADDPATH.
    % cifti_connectivity/src is needed for cifti_conn_matrix_for_wrapper_continous
    % (line 340 below) and is added via MATLAB_ADDPATH in config.sh.
    support_folder = code_dir;
    ...
    if final_outputs_found ==1
        ...
        if run_infomap_too ==1
            ...
            if exist([expected_final_dtseries_file],'file') == 2
                ...
                %surface_only = 0;
                % STANDALONE PCM: community_detection/fair is not bundled.
                % Infomap (run_infomap_too==1) is not used by PCM.
                % addpath(genpath('/projects/standard/faird/.../community_detection/fair'))
